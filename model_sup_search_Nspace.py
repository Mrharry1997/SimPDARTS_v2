...
class MixedOp(nn.Module):
    ...
    def __init__(self, C, stride, switch, p, reduction):
        ...
        for i in range(len(switch)):
            if switch[i]:
                if len(switch) == len(NORMAL_SPACE):
                    primitive = NORMAL_SPACE[i]
                else:
...
class Network(nn.Module):
    ...
    def __init__(self, C, num_classes, layers, criterion, pre_layer, init_layers, total_layers, steps=4, multiplier=4, stem_multiplier=3, switches_normal=[], switches_reduce=[], p=0.3):
        ...
        for i in range(len(switches_reduce)):
            ...
            ons = 0
        self.switch_on_normal = switch_ons_normal[0]
        self.switch_on_reduce = switch_ons_reduce[0]
    ...
    def _initialize_alphas(self):
        ...
        num_ops_normal = self.switch_on_normal
        num_ops_reduce = self.switch_on_reduce
        self.alphas_normal = nn.Parameter(torch.FloatTensor(1e-3*np.random.randn(k, num_ops_normal)))
        self.alphas_reduce = nn.Parameter(torch.FloatTensor(1e-3*np.random.randn(k, num_ops_reduce)))
